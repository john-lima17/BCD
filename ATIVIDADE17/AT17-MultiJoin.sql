...
WHERE tb_cliente.COR_id_cor_fovorita = tb_roupa.COR_id_cor
...
left join tb_pedido_roupa on tb_pedido_roupa.CLIENTE_id_cliente = tb_cliente.id_cliente
where tb_pedido_roupa.CLIENTE_id_cliente is null;

-- exemplo 3
SELECT
c1.nome AS Cateforia,
...
SELECT 
    ...
    tb_roupa.nome AS nome_roupa
FROM
    tb_categoria AS categoria_principal
        LEFT JOIN
    tb_categoria AS SubCategoria ON SubCategoria.CATEGORIA_id_categoria_pai = categoria_principal.id_categoria
        JOIN
    tb_roupa ON tb_roupa.CATEGORIA_id_categoria = SubCategoria.id_categoria
        OR tb_roupa.CATEGORIA_id_categoria = categoria_principal.id_categoria;

-- exemplo 7
CREATE VIEW vw_cores_populares AS
SELECT tb_cor.nome AS cor,
sum(tb_pedido_roupa.quantidade) AS "total"
FROM tb_pedido_roupa
JOIN tb_roupa ON tb_pedido_roupa.ROUPA_id_roupa = tb_roupa.id_roupa
JOIN tb_cor ON tb_roupa.COR_id_cor = tb_cor.id_cor
GROUP BY tb_cor.nome;

SELECT * FROM vw_cores_populares;

-- exemplo 8
SELECT * FROM vw_cores_polulares
WHERE "total" > 10;
